...
type RestOf<T extends IMachine | NewMachine> = Omit<T, 'createdAt' | 'updatedAt'> & {
...
export class MachineService {
  ...
  protected convertDateFromClient<T extends IMachine | NewMachine | PartialUpdateMachine>(machine: T): RestOf<T> {
    return {
      ...machine,
      createdAt: machine.createdAt?.toJSON() ?? null,
      updatedAt: machine.updatedAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMachine: RestMachine): IMachine {
    return {
      ...restMachine,
      createdAt: restMachine.createdAt ? dayjs(restMachine.createdAt) : undefined,
      updatedAt: restMachine.updatedAt ? dayjs(restMachine.updatedAt) : undefined,
    };
  }
